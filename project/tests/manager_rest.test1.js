...
describe("manager test - as manager", () =>{
    ...
    it("successfully set schedule (of games)", async ()=>{
        num_of_games = await DButils.execQuery(`select gameID from games`);
    ...
    it("input 1 rounds should add 66 games to games table", async ()=>{
        let updated_num_of_games = await DButils.execQuery(`select gameID from games`);
        expect(updated_num_of_games.length - num_of_games.length).toBe(66);
    });
